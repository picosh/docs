...
> feeds.pico.sh is now a [pico+](/plus) service, [read our announcement](https://blog.pico.sh/ann-033-moving-rss-to-email-pico-plus)
...
Use our [plain text lists spec](/plain-text-lists) to create a txt file named `daily.txt` (as an example).
...
Control when a digest is sent using the popular cron format. The host server runs on the **UTC timezone** so adjust your cron timers accordingly.

> We don't support cron's less than every 5 minutes: `*/5 * * * *`

See docs for the cron impl we use: [adhocore/gronx](https://github.com/adhocore/gronx?tab=readme-ov-file#cron-expression)
...
We provide a remote CLI that users can use to perform a couple of simple but powerful operations
...
Sometimes a feed digest post will fail to send an email digest. Here are some common reasons:
...
We have two primary ways for a customer to debug why their email digests are not being sent: [logs](/logs) and the feeds [remote cli](#remote-cli).

We provide the ability for users to immediately run their feed post, ignoring digest interval validation. This will also show all logs related to fetching that post.

```bash
ssh feeds.pico.sh run {filename}
```

If you run this command and still do not understand why your email digest isn't being sent, then please [contact us](/contact).
...
There are two main options for removing a post: [remote CLI](#remote-cli) and [sftp](/file-uploads#how-do-i-delete-files).

# Reddit

Yes, but you need to use the RSS links from `https://old.reddit.com`. The new reddit doesn't appear to work properly.
...
However, you should read our [privacy policy](/privacy) and [terms of service](/ops).

Posts are not accessible to the public and we provide no endpoints to view these posts.
